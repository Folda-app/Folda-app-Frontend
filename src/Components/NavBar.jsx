        ...
        <div className={`z-10  w-full  top-0 sticky  ${isOnBoardPage ? 'md:hidden' : 'block'} ${darkColorScheme ? 'bg-PB900' : 'bg-white'} md:border-b `}>
           <div className='flex justify-between items-center py-3 px-4 md:px-5 lg:px-12'>
            <a href="/" className='flex items-center gap-2 py-2  '>
                <img src={Logo} alt="logo" className='lg:w-14 lg:h-12 h-10 w-12 ' />
                <h1 className='font-bold text-4xl lg:text-5xl tracking-tight text-cyan-500 font-Lato'>f<span className='text-orange-400'>o</span>lda</h1>
            </a>
            <div>
            <div className=' hidden md:flex '>
               <ul className='flex p-0 lg:gap-8 md:gap-4 justify-center items-center'>
                {
                    NavItems.map((item, idx)=>{
                        ...
                        return(
                            <li
                            key={idx}
                            className={`lg:text-base md:text-xs font-Gilroy text-Gray700 ${
            ...
            <div className={`w-10 h-10 p-2 rounded-full border border-teal-300 justify-center items-center gap-2.5 flex md:hidden 
             ${
              isToggleOpen
            ...
            >
              {NavItems.map((navItem) => (
                <li className='flex items-stretch'>
                  <Link to={navItem.path} className='flex items-center gap-2 py-4 text-emerald-950 transition-colors duration-300 focus:bg-emerald-50 focus:outline-none focus-visible:outline-none font-Gilroy'>{navItem.name}</Link>
                </li>
              ))}
